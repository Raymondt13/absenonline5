...
<hr>';
    ...
    $query_sakit_tanpa = "SELECT presence_id FROM presence WHERE employees_id='$row_user[id]' AND $filter AND present_id='4' ORDER BY presence_id";
    $sakit_tanpa = $connection->query($query_sakit_tanpa);
    ...
    $query_izin_uu = "SELECT presence_id FROM presence WHERE employees_id='$row_user[id]' AND $filter AND present_id='5' ORDER BY presence_id";
    $izin_uu = $connection->query($query_izin_uu);

    $query_telat = "SELECT presence_id FROM presence WHERE employees_id='$row_user[id]' AND $filter AND present_id='1' AND time_in>'$shift_time_in'";
...
<div class="row">
  ...
  <div class="col-md-3">
    <p>Sakit Dengan Surat : <span class="badge badge-warning">' . $sakit->num_rows . '</span></p>
  </div>

  <div class="col-md-3">
    <p>Sakit Tanpa Surat : <span class="badge badge-warning">' . $sakit_tanpa->num_rows . '</span></p>
  </div>
  ...
  <div class="col-md-3">
    <p>Izin Sesuai Undang Undang : <span class="badge badge-info">' . $izin_uu->num_rows . '</span></p>
  </div>
</div>
</div>'; ?>
...
<?php
  ...
  case 'add-cuty':
    ...
    if (empty($_POST['cuty_end'])) {
      $error[] = 'Tanggal selesai cuti tidak boleh kosong';
    } else {
    ...
    if (empty($_POST['date_work'])) {
      $error[] = 'Tanggal kerja tidak boleh kosong';
    } else {
    ...
    // ==============================
    // HITUNG CUTY TOTAL OTOMATIS
    // ==============================
    if (empty($error)) {

      $start = new DateTime($cuty_start);
      $end   = new DateTime($cuty_end);

      // validasi tanggal
      if ($end < $start) {
        $error[] = 'Tanggal selesai cuti tidak boleh lebih kecil dari tanggal mulai';
      } else {
        // +1 supaya hari pertama ikut dihitung
        $cuty_total = $start->diff($end)->days + 1;
      }
    }
    ...
    if (empty($error)) {
      ...
      // (opsional) cek cuti dalam bulan yang sama
      $month = date('m', strtotime($cuty_start));
      $query = "SELECT cuty_id FROM cuty 
              WHERE employees_id='{$row_user['id']}'
              AND MONTH(cuty_start)='$month'";

      $result = $connection->query($query);

      // kalau mau aktifkan validasi 1x cuti per bulan
      // if ($result->num_rows > 0) {
      //   echo 'Anda sudah mengajukan cuti di bulan ini';
      //   exit;
      // }
  ...
  case 'update-cuty':
    ...
    if (empty($_POST['cuty_id'])) {
      $error[] = 'ID cuti tidak boleh kosong';
    } else {
  ...
  <div class="col-6 col-md-3 mb-2">
      <a href="javascript:void(0)" class="item">
          <div class="detail">
              ...
              <div>
                  <strong>Izin Sesuai Undang Undang</strong>
                  <p>' . $izin_uu->num_rows . ' Hari</p>
              </div>
          </div>
      </a>
  </div>
  ...
  <div class="col-6 col-md-3">
      <a href="javascript:void(0)" class="item">
          <div class="detail">
              ...
              <div>
                  <strong>Sakit dengan surat</strong>
  ...
  <div class="col-6 col-md-3">
      <a href="javascript:void(0)" class="item">
          <div class="detail">
              ...
              <div>
                  <strong>Sakit tanpa surat</strong>
                  <p>' . $sakit_tanpa->num_rows . ' Hari</p>
              </div>
          </div>
      </a>
  </div>
