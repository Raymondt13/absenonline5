...
$('.add-gaji').submit(function (e) {
    if($('input[type=text]').val()=='' && $('textarea.address').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/gaji/proses.php?action=add",
            type: "POST",
            ...
            success: function (data) {
                if (data == 'success') {
                    swal({title: 'Berhasil!', text: 'Data gaji  berhasil disimpan.!', icon: 'success', timer: 1500,});
                   $('#modalAdd').modal('hide');
...
$('.update-gaji').submit(function (e) {
    if($('#txtname').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/gaji/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"sw-mod/gaji/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
