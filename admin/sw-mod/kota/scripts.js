...
/* ----------- Add ------------*/
$('.add-kota').submit(function (e) {
    if($('input[name=namakota]').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/kota/proses.php?action=add",
            type: "POST",
            ...
            success: function (data) {
                if (data == 'success') {
                    swal({title: 'Berhasil!', text: 'Data kota berhasil disimpan.!', icon: 'success', timer: 1500,});
                   $('#modalAdd').modal('hide');
...
$('.update-kota').submit(function (e) {
    if($('#txtnamakota').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"sw-mod/kota/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"sw-mod/kota/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
